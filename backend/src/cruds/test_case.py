...
def get_test_case_by_id(db: Session, campaign_id: int, test_case_id: int):
    ...
    return (
        ...
        .filter(
            ...
            models.TestCase.testCaseID == test_case_id,
        )
        .first()
    )
...
def create_test_case(db: Session, test_case: TestCaseCreate, campaign_id: int):
    ...
    log.info(
        ...
        + f" id={db_test_case.testCaseID} campaign={db_test_case.testCampaignID}"
    )
    db.add(db_test_case)
    db.commit()
    ...
    result = (
        ...
        .update(
            values={
                ...
                models.TestCase.endTime: datetime.now(),
            }
        )
    )
    if result is None:
...
def fail_test_case(
    ...
    db_fail_info = models.FailInfo(
        testCaseID=test_case_id, message=fail_info.message, reportTime=datetime.now()
    )
    log.info(
        f"Created fail info, testcase={db_fail_info.testCaseID} id={db_fail_info.FailInfoID} data={db_fail_info.message}"
    )
    db.add(db_fail_info)
    db.commit()
